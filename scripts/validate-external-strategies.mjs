...
async function publicGet(path) {
  ...
  for (const base of API_BASES) {
    try {
      ...
      failures.push(`${base}:${r.status}`);
    } catch (e) {
      failures.push(`${base}:${e?.name||'ERR'}`);
    }
  }
  throw new Error(`BINANCE_PUBLIC_ENDPOINTS_UNAVAILABLE ${failures.join(',')}`);
}
...
async function fetchKlines(symbol, interval, target) {
  ...
  while (out.length < target) {
    ...
    const {data:rows,base}=await publicGet(`/api/v3/klines?${q}`);
  ...
  return {sourceBase,rows:out.slice(-target).map(k => ({
...
function metrics(trades){
  ...
  return {trades:trades.length,profitFactor:grossLoss>0?grossProfit/grossLoss:(grossProfit>0?999:0),expectancy:rets.length?rets.reduce((a,b)=>a+b,0)/rets.length:0,winRate:rets.length?wins.length/rets.length:0,maxDrawdown:maxDD,totalReturn:equity-1};
}

function runEmaPhase(hourly){
  const c=hourly.map(x=>x.close),ef=ema(c,10),es=ema(c,20),s50=sma(c,50),s200=sma(c,200);const trades=[];let pos=null;
  for(let i=201;i<hourly.length-1;i++){
    const bull=c[i]>s50[i]&&c[i]>s200[i];
    if(!pos&&bull&&crossAbove(ef,es,i)){const entry=hourly[i+1].open*(1+SLIPPAGE);pos={entry,entryTime:hourly[i+1].t};continue;}
    if(pos&&crossBelow(ef,es,i)){const exit=hourly[i+1].open*(1-SLIPPAGE);const gross=exit/pos.entry-1;trades.push({...pos,exit,exitTime:hourly[i+1].t,grossReturn:gross,netReturn:gross-2*FEE});pos=null;}
  }
...
function runDualMomentum(hourly,daily){
  const hc=hourly.map(x=>x.close),hs50=sma(hc,50),hs200=sma(hc,200),hr=roc(hc,60);const dc=daily.map(x=>x.close),ds50=sma(dc,50),ds200=sma(dc,200),dr=roc(dc,60);const trades=[];let pos=null;
  for(let i=201;i<hourly.length-1;i++){
    const di=dailyIndexAt(daily,hourly[i].t);if(di<200)continue;
    const currentBull=hc[i]>hs50[i]&&hs50[i]>hs200[i]&&hr[i]>0;const dailyBull=dc[di]>ds50[di]&&ds50[di]>ds200[di]&&dr[di]>0;const signal=currentBull&&dailyBull;
...
const report={generatedAt:new Date().toISOString(),source:'Binance public klines with endpoint failover',assumptions:{feePerSide:FEE,slippagePerSide:SLIPPAGE,execution:'next-bar-open',closedCandlesOnly:true,roundTripDrag:ROUND_TRIP_DRAG},gate,symbols:{}};
for(const symbol of SYMBOLS){
  const [hourPack,dailyPack]=await Promise.all([fetchKlines(symbol,'1h',4000),fetchKlines(symbol,'1d',500)]);const hourly=hourPack.rows,daily=dailyPack.rows;
  const emaTrades=runEmaPhase(hourly),dualTrades=runDualMomentum(hourly,daily);const emaM=metrics(emaTrades),dualM=metrics(dualTrades);
  report.symbols[symbol]={sourceBases:{hourly:hourPack.sourceBase,daily:dailyPack.sourceBase},bars:{hourly:hourly.length,daily:daily.length},strategies:{bec_ema_cross_with_market_phases:{metrics:emaM,decision:decide(emaM)},bec_dual_momentum_simple:{metrics:dualM,decision:decide(dualM)}}};
}
for(const id of ['bec_ema_cross_with_market_phases','bec_dual_momentum_simple']){const perSymbol=Object.entries(report.symbols).map(([symbol,s])=>({symbol,...s.strategies[id]}));report[id]={perSymbol,validatedAcrossAll:perSymbol.every(x=>x.decision.validated)};}
await fs.mkdir('validation/external-strategies',{recursive:true});await fs.writeFile('validation/external-strategies/latest.json',JSON.stringify(report,null,2));console.log(JSON.stringify(report,null,2));
