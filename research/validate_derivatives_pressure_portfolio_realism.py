#!/usr/bin/env python3
"""Portfolio-realism validation for frozen derivatives-pressure 48h strategy.

Tests the existing frozen signal without threshold changes under practical constraints:
- next 15m bar open entry
- fixed 48h exit
- no overlapping position in the same symbol
- max 3 concurrent positions portfolio-wide
- equal notional per accepted trade
- 20/40/60 bps round-trip cost scenarios
...
OUT=pathlib.Path('validation/edges/derivatives-pressure-48h-portfolio-realism.json')
...
def get(base,path):
    req=urllib.request.Request(base+path,headers={'User-Agent':'Mozilla/5.0 tst-portfolio-realism/1.0','Accept':'application/json,text/plain,*/*','Referer':'https://www.binance.com/'})
...
for s in SYMBOLS:
    try:
        ...
        for ts in sorted(set(om)&set(tm)&set(sm)):
            ...
            t={'symbol':s,'entryTs':entry_ts,'exitTs':exit_ts,'gross':gross,'oiChange2h':oichg,'takerBuySellRatio1h':tr}
            for n,c in COSTS.items(): t[n]=gross-c
            raw.append(t)
    except Exception as e: failures[s]=str(e)

raw.sort(key=lambda x:(x['entryTs'],x['symbol']))
accepted=[]; rejected={'sameSymbolOverlap':0,'portfolioCapacity':0}; open_positions=[]
for t in raw:
    open_positions=[p for p in open_positions if p['exitTs']>t['entryTs']]
    if any(p['symbol']==t['symbol'] for p in open_positions):
        rejected['sameSymbolOverlap']+=1; continue
    if len(open_positions)>=MAX_CONCURRENT:
        rejected['portfolioCapacity']+=1; continue
...
# Simple equal-notional realized equity curve by exit time. Each trade contributes its net return / MAX_CONCURRENT.
...
report={'schemaVersion':1,'strategyId':'TST_DERIVATIVES_PRESSURE_V2_48H_EXECUTABLE_V1','validationType':'PORTFOLIO_REALISM','authorization':'RESEARCH_ONLY','liveTrading':False,'constraints':{'maxConcurrentPositions':MAX_CONCURRENT,'sameSymbolOverlapAllowed':False,'holdingPeriodHours':48},'rawSignals':len(raw),'acceptedTrades':len(accepted),'rejected':rejected,'scenarios':scenarios,'portfolio':{'endingEquityMultiple':round(equity,4),'maxDrawdownPct':round(maxdd*100,2)},'validation':validation,'failures':failures,'generatedAt':now.isoformat()}
