#!/usr/bin/env python3
"""Gate G V1: true L2 OBI/microprice diagnostic from Bybit ob500 archives.
Research-only feature validation; NOT a standalone trading strategy.
Frozen event definitions, no ML, sizing, or live execution.
"""
...
def main():
    ...
    with z.open(name) as f:
        for raw in f:
            ...
            spread_bps=(apx-bp)/mid*1e4
            samples.append((ts,mid,obi,micro_bps,spread_bps))
            next_sample=((ts//1000)+1)*1000
    # 1-second samples; fixed long/short confirmation events.
    arr=np.array(samples,float);outcomes={}
    for side in ('long','short'):
        outcomes[side]={}
        mask=(arr[:,2]>=0.60)&(arr[:,3]>=0.08) if side=='long' else (arr[:,2]<=-0.60)&(arr[:,3]<=-0.08)
        idx=np.flatnonzero(mask);# non-overlap 60s to reduce pseudo-replication
        keep=[];last=-999999
        for i in idx:
    ...
    result={'engine':'GATE_G_L2_MICROSTRUCTURE_V1','authorization':'RESEARCH_ONLY','liveTrading':False,'usesML':False,'usesSizing':False,'thresholdsFrozen':True,'symbol':a.symbol,'date':a.date,'archiveBytes':len(blob),'sampleCount':len(samples),'eventDefinition':{'obi10AbsMin':0.60,'micropriceDirectionalBpsMin':0.08,'sampleSeconds':1,'eventCooldownSeconds':60},'outcomes':outcomes}
