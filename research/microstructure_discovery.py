...
1m klines for taker-buy flow and forward returns. This intentionally omits L2/book
data in phase 1 because tick-level book_ticker samples are very large; only if this
phase shows signal separation should we pay the compute/data cost for L2.
"""
...
from tardis_sample_loader import load_monthly_sample
...
def minute_ms(ts_ms: int) -> int:
    return (int(ts_ms) // MIN_MS) * MIN_MS
...
for y, mo in MONTHS:
    ...
    for symbol in SYMBOLS:
        ...
        try:
            liq_rows = load_monthly_sample("liquidations", y, mo, symbol)
            liqs = defaultdict(lambda: {"liqNotional": 0.0, "buyLiq": 0.0, "sellLiq": 0.0, "liqCount": 0})
            for r in liq_rows:
                ts = r.get("timestamp_ms") or r.get("local_timestamp_ms")
                if ts is None:
                    continue
                m = minute_ms(int(ts))
                px = f(r.get("price"))
            ...
            data_counts[key] = {"minutes": len(day_minutes), "liquidationMinutes": len(liqs), "liquidationEvents": len(liq_rows)}
...
# Directional regimes. Binance liquidation side identifies forced order side;
# we keep the labels descriptive and avoid assuming economic interpretation.
