...
def normalize_message(payload: dict, received_ms: int) -> dict | None:
    ...
    if price <= 0 or qty <= 0 or side not in {"BUY", "SELL"}:
        return None
    # BUY force-order closes a short; SELL force-order closes a long.
    ...
    def __init__(self, path: pathlib.Path):
        ...
        self.con.execute("""
            CREATE TABLE IF NOT EXISTS liquidations (
              ...
              symbol VARCHAR,
              order_side VARCHAR,
              liquidation_side VARCHAR,
              price DOUBLE,
              quantity DOUBLE,
              notional DOUBLE,
              status VARCHAR,
              raw_order_type VARCHAR,
...
def self_test() -> None:
    ...
    assert row and row["symbol"] == "BTCUSDT"
    assert row["liquidation_side"] == "LONG"
    assert row["quantity"] == 2.0
    assert abs(row["notional"] - 83980.0) < 1e-9
