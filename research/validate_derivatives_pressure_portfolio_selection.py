#!/usr/bin/env python3
"""Portfolio-selection validation for frozen derivatives-pressure 48h strategy.

Keeps the signal thresholds frozen. The only implementation rule added is deterministic
selection when more candidates arrive than the portfolio can hold: rank each same-bar
batch by pressure strength (score, OI change, taker ratio), then fill up to 3 open slots.
No same-symbol overlap. Entry next 15m open; exit exactly 48h later. Research only.
"""
...
OUT=pathlib.Path('validation/edges/derivatives-pressure-48h-portfolio-selection.json')
...
def get(base,path):
    req=urllib.request.Request(base+path,headers={'User-Agent':'Mozilla/5.0 tst-portfolio-selection/1.0','Accept':'application/json,text/plain,*/*','Referer':'https://www.binance.com/'})
...
report={'schemaVersion':1,'strategyId':'TST_DERIVATIVES_PRESSURE_V2_48H_EXECUTABLE_V1','validationType':'PORTFOLIO_SELECTION_RANKED_BATCH','authorization':'RESEARCH_ONLY','liveTrading':False,'selectionPolicy':'same-entry-bar candidates ranked by frozen pressure score, then OI change, then taker ratio','constraints':{'maxConcurrentPositions':MAX_CONCURRENT,'sameSymbolOverlapAllowed':False,'holdingPeriodHours':48},'rawSignals':len(raw),'acceptedTrades':len(accepted),'rejected':rejected,'scenarios':scenarios,'portfolio':{'endingEquityMultiple':round(equity,4),'maxDrawdownPct':round(maxdd*100,2)},'validation':validation,'failures':failures,'generatedAt':now.isoformat()}
