...
def metrics(x) -> dict:
    ...
    p = float(stats.binomtest(wins, nonzero, p=0.5, alternative="greater").pvalue) if nonzero else 1.0
    return {
        "n": int(len(a)),
...
def main() -> None:
    ...
    supporting = sum(
        1 for x in by_symbol
        if x["n"] >= 2 and x["medianShortReturn"] is not None and x["medianShortReturn"] > 0 and x["shortHitRate"] > 0.5
    )
    gate = bool(
    ...
    payload = {
        ...
        "candidate":{
            "feature":"agg_trade_count",
            "tail":"HIGH",
            "threshold":"symbol-specific q90 frozen from discovery calibration",
            "expectedDirection":"DOWN / SHORT-direction",
            "horizon":"4h",
            "declusterBars":HORIZON_BARS,
            "declusterMinutes":240,
        },
        ...
        "gate":{
            "minPooledEvents":MIN_POOLED_EVENTS,
            "shortHitRate":">0.60",
            "meanShortReturn":">0",
            "medianShortReturn":">0",
            "signTestP":"<0.05",
            "minSupportingSymbols":MIN_SYMBOLS_SUPPORT,
            "pass":gate,
        },
        "events":all_events,
