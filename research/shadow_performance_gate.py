#!/usr/bin/env python3
"""Research-only performance gate for labeled shadow trades."""
...
def gate(m):
    ...
    return {'pass':not reasons,'reasons':reasons,'liveAuthorized':False,'next':'WALK_FORWARD_REVIEW' if not reasons else 'KEEP_SHADOW'}
def selftest():
    rows=[{'status':'CLOSED','net_return_pct':'0.8'} for _ in range(35)]+[{'status':'CLOSED','net_return_pct':'-0.5'} for _ in range(15)]
    m=metrics(rows); g=gate(m); assert m['n']==50 and g['liveAuthorized'] is False
    print(json.dumps({'selftest':'PASS','metrics':m,'gate':g}))
def main():
    ...
    rows=list(csv.DictReader(a.input.open(encoding='utf-8')));o={'engine':'SHADOW_PERFORMANCE_GATE_V1','authorization':'RESEARCH_ONLY','liveTrading':False,'metrics':metrics(rows)};o['gate']=gate(o['metrics']);a.output.parent.mkdir(parents=True,exist_ok=True);a.output.write_text(json.dumps(o,indent=2));print(json.dumps(o))
