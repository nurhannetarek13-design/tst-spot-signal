...
app.get("/", (_req, res) => {
  res.json({
    ...
    mode: "SIGNAL_ONLY",
    liveTrading: false,
    endpoints: ["/health", "/scan", "/signal/BTCUSDT"],
    risk: CFG,
  });
});
  ...
  for (const base of API_BASES) {
    try {
      const upstream = await fetch(`${base}/api/v3/time`, {
        headers: { Accept: "application/json", "User-Agent": "tst-spot-signal-vercel/2.0" },
...
async function analyze(symbol, ticker, book, rawKlines) {
  ...
  return {
    ...
    market: {
      ...
      quoteVolume24hUSDT: round(Number(ticker.quoteVolume || 0), 0),
      spreadPct: round(spreadPct, 4),
    },
    indicators: {
      ema20: round(ema20, 8), ema50: round(ema50, 8), rsi14: round(rsi14, 2), atr14: round(atr, 8), volumeRatio20: round(volumeRatio, 2), resistance20: round(resistance20, 8), support20: round(support20, 8),
    },
    checks: { trendUp, breakout, nearBreakout, healthyMomentum, volumeConfirm, liquid, spreadOk, notChasing },
    paperPlan: decision === "BUY" ? {
      entry: round(entry, 8),
      stop: round(stop, 8),
      target1: round(target1, 8),
      maxPositionUSDT: round(positionUSDT, 2),
      maxRiskUSDT: CFG.maxRiskPerTradeUSDT,
      estimatedRoundTripFeesUSDT: round(estFees, 4),
    } : null,
    liveTrading: false,
  };
}
