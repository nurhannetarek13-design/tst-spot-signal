...
async function discoverAndRepairBotPositions() {
  ...
  for (const symbol of candidates) {
    ...
    const entry = (orders || [])
      .filter(o => o.side === "BUY" && Number(o.executedQty || 0) > 0 && /^TST[BWQ][a-f0-9]{8}$/i.test(String(o.clientOrderId || "")))
    ...
    const exits = (orders || []).filter(o =>
      ...
      new RegExp(`^TST[TSUVX]${token}$`, "i").test(String(o.clientOrderId || ""))
    );
...
app.post("/executor/position-status", async (req, res) => {
  try {
    ...
    const exitOrder = (orders || [])
      ...
      .filter(o => /^TST[TSUVX]/.test(String(o.clientOrderId || "")))
...
async function botClosedPnlLast24h(knownSymbols = []) {
  ...
  for (const symbol of normalizeKnownSymbols(knownSymbols, "")) {
    ...
    for (const order of filled) {
      ...
      const match = id.match(/^TST[BWQ]([a-f0-9]{8})$/i);
      if (match && order.side === "BUY") buys.set(match[1].toLowerCase(), order);
    }
    for (const order of filled) {
      const id = String(order.clientOrderId || "");
      const match = id.match(/^TST[TSUVX]([a-f0-9]{8})$/i);
...
async function executeSpotOrder(symbol, signalHash, knownSymbols = [], approvedSignal) {
  ...
  // Prefer Binance OPOCO first. OPOCO is exposed by Binance Spot's signed
  ...
  // Prefer Binance OTOCO when the symbol supports it. The working BUY is a
  ...
  const tokenExitOrders = (existingOrders || []).filter(order =>
    order.side === "SELL" && new RegExp(`^TST[TSUVX]${token}$`, "i").test(String(order.clientOrderId || ""))
  );
...
export { isStepAligned, validateOrderFilters, isUnknownExecutionError, normalizeKnownSymbols, validateApprovedSignal, isTerminalOrder, canUseOpoco, buildOpocoFokPlan, canonicalWsPayload, canUseOtoco, buildOtocoFokPlan };
export default app;
