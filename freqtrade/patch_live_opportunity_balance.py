...
# Live opportunity balance patch.
# Fixes two concrete starvation paths seen in production logs:
# 1) adaptive-watch -> direct promotion was being followed by another independent
#    2-hit confirmation, effectively requiring three observations;
# 2) WEAK_BEAR blocked the NORMAL controlled-breakout lane completely even when
#    symbol-specific momentum/flow was exceptional and BTC hard-risk had cleared.
#
# This patch does NOT relax PANIC, OCO, live R:R, order-flow/profile/Wyckoff,
# portfolio risk, daily loss, spread, liquidity, or product exclusions.
...
new_direct = r'''def _direct_confirmation_ready(symbol: str, score: float, m: dict) -> bool:
    ...
    # Adaptive watch already is a first independent observation.  If the same
    # symbol later PROMOTEs to DIRECT after the minimum age and has not faded,
    # count that promotion as the second observation instead of arming a third.
    prior = None
    ...
    if prior is not None and float(score) >= DIRECT_SCORE:
        ...
        if first_score >= WATCH_SCORE and age_ok and price_ok:
            ...
            print(
                f'[direct-confirm] PASS {symbol} score={score:.0f} '
                f'source=adaptive-watch-promotion age={age:.0f}s -> final gates'
            )
            return True
...
for marker in [
    'source=adaptive-watch-promotion',
    'first_score >= WATCH_SCORE',
    'DIRECT_CONFIRM_MIN_SEC <= age',
]:
    if marker not in s:
        raise SystemExit(f'live-opportunity-balance: fast marker missing {marker}')
compile(s, str(fast_path), 'exec')
fast_path.write_text(s, encoding='utf-8')

# ---------------------------------------------------------------------------
# Let only exceptional NORMAL controlled breakouts survive WEAK_BEAR context.
# The dedicated market-structure/flow gate and reduced weak-bear sizing still run
# downstream, so this removes a contradictory blanket veto rather than opening
# the market broadly.
# ---------------------------------------------------------------------------
...
if 'FALLBACK_WEAK_BEAR_NORMAL_SCORE =' not in g:
    ...
    g = g.replace(
        const_marker,
        const_marker + "FALLBACK_WEAK_BEAR_NORMAL_SCORE = float(os.getenv('SPOT_SNIPER_WEAK_BEAR_NORMAL_SCORE', '96'))\n",
        1,
    )
...
new = """    elif regime == 'WEAK_BEAR':
        # Allow only exceptional NORMAL first-break/controlled-continuation,
        # plus the already-approved MID/REVERSAL lanes.  EXPLOSIVE/EXTREME stay
        # blocked in a weak tape.
...
# In a thin market-context snapshot, NORMAL score 98+ is strong enough to reach
# the downstream symbol-specific gates even when that symbol was absent from the
# cross-sectional sample.  This is still stricter than the normal 96 threshold.
...
for marker in [
    'FALLBACK_WEAK_BEAR_NORMAL_SCORE =',
    "lane not in {'NORMAL', 'MID', 'REVERSAL'}",
    'weakbear_normal =',
    'max(required_score, 98.0)',
]:
    if marker not in g:
        raise SystemExit(f'live-opportunity-balance: gate marker missing {marker}')
compile(g, str(gate_path), 'exec')
gate_path.write_text(g, encoding='utf-8')

print('[live-opportunity-balance] OK watch-promotion counts as persistence + WEAK_BEAR NORMAL score>=96 (unranked>=98); panic/explosive/extreme unchanged')
