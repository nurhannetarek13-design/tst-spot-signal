...
# Runtime whipsaw + profit-giveback protection for the fully patched Spot Sniper.
# This version targets the post-ignition/post-growth fast_entry_engine layout.
...
if 'DIRECT_CONFIRM_HITS =' not in s:
    if const_marker not in s:
        raise SystemExit('whipsaw-v2 failed: WATCH_TTL_SEC marker missing')
...
if 'direct_confirm_state: dict[str, dict]' not in s:
    if state_marker not in s:
        raise SystemExit('whipsaw-v2 failed: adaptive watch state marker missing')
...
if 'def _whipsaw_stop_pct(' not in s:
    if helper_marker not in s:
        raise SystemExit('whipsaw-v2 failed: helper marker missing')
...
if loop_header_old in s:
    ...
    raise SystemExit('whipsaw-v2 failed: direct loop header missing')
...
if 'if not _direct_confirmation_ready(symbol, score, m):' not in s:
    if signal_call_old not in s:
        raise SystemExit('whipsaw-v2 failed: maybe_signal call marker missing')
    s = s.replace(signal_call_old, signal_call_new, 1)

# Post-ignition/post-growth layout. Preserve its larger adaptive target, but move
# the stop outside ordinary 1m noise and then enforce a minimum reward:risk.
stop_old = (
...
stop_new = (
    ...
    "    tp_pct = clamp(max(tp_pct, sl_pct * WHIPSAW_MIN_RR), 0.0150, 0.0350)\n"
    "    entry = m['live_price']\n"
)
if 'sl_pct = _whipsaw_stop_pct(sl_pct, m)' not in s:
    if stop_old not in s:
        raise SystemExit('whipsaw-v2 failed: ignition TP/SL marker missing')
    s = s.replace(stop_old, stop_new, 1)

for item in [
    'DIRECT_CONFIRM_HITS =', 'WHIPSAW_MIN_STOP_PCT =',
    'def _direct_confirmation_ready(', 'def _whipsaw_stop_pct(',
    'if not _direct_confirmation_ready(symbol, score, m):',
    'sl_pct = _whipsaw_stop_pct(sl_pct, m)',
]:
    if item not in s:
        raise SystemExit(f'whipsaw-v2 failed: missing fast marker {item}')
compile(s, str(fast_path), 'exec')
fast_path.write_text(s, encoding='utf-8')

# ---------------------------------------------------------------------------
# Dynamic exit: post-entry peak only + profit giveback lock.
# ---------------------------------------------------------------------------
...
if 'PROFIT_LOCK_ARM_R =' not in d:
    if ratchet_const not in d:
        raise SystemExit('whipsaw-v2 failed: dynamic constants marker missing')
...
if "'last_red':" not in d:
    if market_return_old not in d:
        raise SystemExit('whipsaw-v2 failed: market return marker missing')
...
if stop_start < 0 or stop_end < 0:
    raise SystemExit('whipsaw-v2 failed: _suggest_stop block missing')
...
if 'candle_high = float(market.get(' not in d:
    if peak_old not in d:
        raise SystemExit('whipsaw-v2 failed: run_once peak marker missing')
...
if '[dynamic-exit] EARLY_LOCK' not in d:
    if seq_marker not in d:
        raise SystemExit('whipsaw-v2 failed: dynamic _replace marker missing')
...
for item in [
    ...
    if item not in d:
        raise SystemExit(f'whipsaw-v2 failed: missing dynamic marker {item}')
compile(d, str(dex_path), 'exec')
dex_path.write_text(d, encoding='utf-8')

print('[early-whipsaw-protection-v2] OK persistent entry + noise stop + post-entry peak + red-reversal/giveback profit lock')
