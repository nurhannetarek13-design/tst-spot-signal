...
ALERTS_ENABLED = str(os.getenv('SOL_BUY_ZONE_ALERTS_ENABLED', '1')).strip().lower() in {'1', 'true', 'yes', 'on'}
...
def evaluate() -> dict:
    ...
    if last1h['close'] < PULLBACK_LOW:
        return {'trigger': None, 'price': price, 'reason': '1h_below_invalidation', 'invalidation': PULLBACK_LOW}
    ...
    if touched_zone and bullish_rejection and improving and buyer_confirm and price >= PULLBACK_LOW:
        return {'trigger': 'PULLBACK_REJECTION', 'price': price, 'close15': last['close'], 'low15': last['low'], 'taker_buy_ratio': last['taker_buy_ratio'], 'invalidation': PULLBACK_LOW}
    ...
    if breakout_hold and breakout_momentum and (crossed or last['low'] <= BREAKOUT_LEVEL):
        return {'trigger': 'BREAKOUT_HOLD', 'price': price, 'close15': last['close'], 'low15': last['low'], 'taker_buy_ratio': last['taker_buy_ratio'], 'invalidation': BREAKOUT_LEVEL}
    return {'trigger': None, 'price': price, 'reason': 'no_confirmed_setup', 'last15_close': last['close'], 'taker_buy_ratio': last['taker_buy_ratio']}
...
def main() -> None:
    if not ALERTS_ENABLED:
        print('[sol-buy-zone] DISABLED confirmed-buy-only mode', flush=True)
        while True:
    ...
    while True:
        try:
            ...
            if should_alert(result, state):
                ...
                print(f'[sol-buy-zone] check price={result.get("price")} trigger={result.get("trigger")} reason={result.get("reason", "")}', flush=True)
