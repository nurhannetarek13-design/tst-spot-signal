...
def _signed_get(path,params):
    ...
    for base in API_BASES:
        try:
            req=urllib.request.Request(f'{base}{path}?{q}&signature={sig}',headers={'X-MBX-APIKEY':k,'User-Agent':'tst-reconciler/1.1'})
...
def run_once():
    ...
    if unresolved:
        raise RuntimeError(f'UNRESOLVED_BOT_OCO:{unresolved}')
    state=trade_state.load_state()
    for signal_id,pos in list((state.get('positions') or {}).items()):
        ...
        row=_signed_get('/api/v3/orderList',{'orderListId':oid}); status=str(row.get('listOrderStatus') or '').upper(); status_type=str(row.get('listStatusType') or '').upper()
        if status in {'ALL_DONE','REJECT'} or status_type in {'ALL_DONE','RESPONSE'}:
            trade_state.update_position(signal_id,status='CLOSED',closed_at=time.time(),close_reason='BINANCE_RECONCILED_DONE'); trade_state.append_event('POSITION_CLOSED',signal_id=signal_id,symbol=pos.get('symbol'),reason='BINANCE_RECONCILED_DONE',order_list_id=oid)
    snap=trade_state.portfolio_snapshot(); print(f"[reconcile] OK bot_open_ocos={len(open_ocos)} tracked_open={snap['open_count']} risk={snap['stop_risk_usdt']:.4f} incomplete={snap['incomplete_count']}",flush=True)

def main():
    print(f'[reconcile] ONLINE read_only=True poll={POLL_SEC}s ownership_prefix=TSTO- fail_closed=True',flush=True)
    while True:
        try: run_once()
