...
# ---------------------------------------------------------------------------
# 1) Every fast lane must persist across multiple scans before it can become a
# Telegram CONFIRMED BUY. NORMAL already has its dedicated whipsaw confirmation;
# this closes the gap for MID / EXPLOSIVE / EXTREME / REVERSAL.
# ---------------------------------------------------------------------------
...
def _lane_confirmation_ready(lane: str, symbol: str, score: float, price: float) -> bool:
    """Require persistent strength; one hot scan can never become CONFIRMED BUY."""
    lane = str(lane or 'NORMAL').upper()
    if lane == 'NORMAL':
        # NORMAL is already protected by _direct_confirmation_ready injected by
        # the whipsaw patch. Do not double-delay it.
        return True
    now = time.time()
    key = f'{lane}:{symbol}'
    ...
    # Do not accept a second scan that has already materially broken down from
    # the first observation. The lane's full score/context gates are rerun on
    # every call before this function is reached.
    ...
    if int(row['hits']) >= LANE_CONFIRM_HITS and age >= LANE_CONFIRM_MIN_SEC and price_ok:
        ...
        return True
    if not price_ok:
        # A >0.5% fade while waiting invalidates the setup rather than counting
        # stale strength toward confirmation.
        _lane_confirm_state.pop(key, None)
...
# ---------------------------------------------------------------------------
# 2) MID is no longer given a second arbitrary 25% haircut after the central
# dynamic risk/balance allocator has already sized it. Score priority remains
# 10/20/30/40; risk, stop distance, free balance and the 40-USDT cap may reduce it.
# Early Reversal intentionally remains starter-sized because it is a different,
# lower-confidence lane and we do not yet pyramid/re-protect atomically.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 3) Production fallback must still be selective cross-sectionally. The old
# fallback allowed rank 31 to pass on score alone. Keep unapproved EV shadow-only,
# but require a candidate to be within a generous top-10 opportunity rank.
# This would preserve ZEC rank 4 while blocking the CRCLB rank-31 setup.
# ---------------------------------------------------------------------------
...
for marker in [
    'def _lane_confirmation_ready(',
    "'persistent-confirmation-pending'",
    'mult = 1.0',
]:
    if marker not in s:
...
for marker in [
    'FALLBACK_MAX_RANK =',
    "'FALLBACK_RANK_REJECT'",
    "strategy.startswith('EARLY_REVERSAL_STARTER')",
]:
    if marker not in g:
...
print('[final-entry-hardening] OK all fast lanes persistent-confirmed; MID uses central priority sizing; fallback rank<=10; reversal classified')
