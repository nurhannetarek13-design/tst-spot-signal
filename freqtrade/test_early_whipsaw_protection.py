...
def test_pre_entry_high_does_not_fake_profit() -> None:
    ...
    market = {
        'bid': 100.05,
        'ask': 100.06,
        'high20': 105.0,  # large high happened before this BUY
        'swing_low7': 99.8,
        'atr': 0.20,
        'atr_pct': 0.0020,
        'last_open': 104.0,
        ...
        'last_low': 99.9,
        'last_close': 100.0,
        'last_open_time': opened - 60.0,
        'last_close_time': opened - 1.0,
        'last_red': True,
    }
...
def test_red_reversal_locks_earned_profit() -> None:
    ...
    market = {
        ...
        'last_low': 100.75,
        'last_close': 100.80,
        'last_open_time': opened + 540.0,
        'last_close_time': opened + 599.0,
        'last_red': True,
    }
    suggested, reason = dex._suggest_stop(pos, market)
    assert suggested is not None, reason
    assert suggested > 100.0, (suggested, reason)
    assert reason in {'red-reversal-profit-lock', 'profit-giveback-lock', 'structure-lock', 'near-target-lock'}, reason
