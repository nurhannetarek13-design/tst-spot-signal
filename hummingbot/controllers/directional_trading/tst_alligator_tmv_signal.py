...
class TSTAlligatorTMVSignalConfig(DirectionalTradingControllerConfigBase):
    ...
    strategy_id: str = Field(default="TST_ALLIGATOR_TMV_V1")
...
class TSTAlligatorTMVSignalController(DirectionalTradingControllerBase):
    """Paper-only Alligator + Momentum + Volume controller.

    Core entry:
      causal Alligator expansion + MACD confirmation + PSAR confirmation
      + relative-volume expansion + price above EMA200.

    Fusion then adds BTC regime, L2, taker-flow and liquidity gates.
    """
    ...
    async def update_processed_data(self):
        ...
        volume_confirmed = bool(pd.notna(relvol.iloc[i]) and relvol.iloc[i] >= 1.20)
        trend_confirmed = bool(pd.notna(ema200.iloc[i]) and close.iloc[i] > ema200.iloc[i])

        core_trigger = alligator_trigger and macd_confirmed and sar_confirmed and volume_confirmed and trend_confirmed
        ...
        if not core_trigger:
            ...
            self.processed_data["signal"] = 0
            self.processed_data["fusion"] = {"decision": "NO_TRADE", "reasons": reasons}
            return
